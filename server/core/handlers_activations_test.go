...
func TestUploadGetActivationJWTV1(t *testing.T) {
	lock := sync.Mutex{}
	...
	expireAt := time.Now().Add(24 * time.Hour).Unix()
	act := jwtv1.NewActivationClaims(acct2PubKey)
	...
	act, err = jwtv1.DecodeActivationClaims(actJWT)
	require.NoError(t, err)
	...
	savedClaims, err := jwtv1.DecodeActivationClaims(string(savedJWT))
	require.NoError(t, err)
	hash2, err := act.HashID()
	require.NoError(t, err)
